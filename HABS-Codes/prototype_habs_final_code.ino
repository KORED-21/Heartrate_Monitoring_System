...
const int sdToCS = 5;
const int buzzer = 6;
const int threshold = 550; //This value determines how sensitive the pulse sensor module will be while it is running on 5v and the user is using
                           //his/her finger to measure pulse.
const int lowBPM = 50;     //low bpm value
const int highBPM = 125;   //high bpm value
...
void loop() {
  ...
  if (pulseModule.sawStartOfBeat()) {          //Tests to see if the signal peaks 550. If the signal goes past 550, then a "beat happened."
    ...
    delay(500); //For stability, data saturation prevention, and longevity of the power supply.
    ...
    if(myBPM<lowBPM||myBPM>highBPM){   //If a person's heartrate goes above highBPM or below lowBPM,
      ...
      if(myBPM<lowBPM||myBPM>highBPM){ //if the same is still true, then ring the buzzer.
